...
class LocalPastDataCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'past:local-data';
...
//        Games::where('date', $game->date)->delete();
        $total = 26;
        for ($i = 1; $i < $total; $i++)  {
//            $game = Games::where('server_id', 1)->orderBy('date')->first();
            $this->date = Carbon::createFromFormat('Y-m-d', '2018-12-31')->addDays($i)->format('Y-m-d');
            dump($i, $this->date);
            $this->exec();
        }
    }
